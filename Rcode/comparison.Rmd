...
set.seed(1)
{
  ...
  c2track <- list()
  c3track <- list()
  al=0.5;bl=2;av=1;bv=10;am=5;bm=1;sigmal=1;mub=-1
...
resultnob = foreach(i=1:50,.combine=rbind,.packages=c("progress","testarma","gtools")) %dopar%
  {
    data <- generate_nonoverlap(i)
    #y <- data[[1]]
    #yna <- data[[2]]
...
mean(resultnob[,1]/15)
sd(resultnob[,1]/15)/sqrt(50)
mean(resultnob[,2]/200)
sd(resultnob[,2]/200)/sqrt(50)
mean(resultnob[,3]/15)
sd(resultnob[,3]/15)/sqrt(50)
```
...
om = foreach(i=1:50,.packages=c("simFrame","progress","MatrixForm","gtools","truncnorm")) %dopar% 
  {
    data <- generatedata_overlap(i)
    y <- data[[1]]
...
mean(om[,2]/15)
sd(om[,2]/15)
nom = foreach(i=1:50,.packages=c("progress","MatrixForm","gtools","truncnorm")) %dopar% 
  {
    data <- generate_nonoverlap(i)
    y <- data[[1]]
    y_matrix <- apply(y, c(1,3),mean)
